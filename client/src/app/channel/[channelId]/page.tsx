...
import { useChannelQuery } from '@/store/api/channel.api'
import { Avatar } from '@radix-ui/react-avatar'
import { List, Recycle, Send, X } from 'lucide-react'
import Image from 'next/image'
import Link from 'next/link'
import { useParams } from 'next/navigation'
import ChannelSkeleton from './channelSkleton'
import { TChannelSingle } from '@/types/channel'
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs'
import { useState } from 'react'
import { Button } from '@/components/ui/button'

interface TTabsProps {
    channel: TChannelSingle | undefined
}

function ActTypes({ channel }: TTabsProps) {
                        ...
                        <Card className="p-4">
                            <div className="grid grid-cols-1 lg:flex lg:justify-between lg:px-6 gap-4 lg:gap-0">
                                ...
                                <div className="col-span-2 lg:col-span-1 lg:w-1/3 lg:flex lg:flex-col lg:items-end">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {channel?.chains?.length ? (
                                            channel.chains.map((e) => (
...
export default function Channel() {
    ...
    const { currentData: channel, isLoading, isFetching } = useChannelQuery(id as string, { skip: !id })
                ...
                <div className="container mx-auto pt-4 pb-8 px-4">
                    <div className="mb-4 relative overflow-hidden rounded-lg">
                        ...
                        <div className="inset-0 z-0">
                            ...
                            >
                                <Image
                                    src={constructImageUrl(channel?.coverPhotoUrl)}
                                    alt="Background"
                        ...
                        <div className="p-8 relative z-20 flex flex-col items-center justify-center">
                            <Avatar className="h-18 w-18">
                                <AvatarImage src={constructImageUrl(channel?.logoUrl)} alt="Logo" />
                                <AvatarFallback className="text-xl">{channel?.name?.slice(0, 2)}</AvatarFallback>
                            </Avatar>
