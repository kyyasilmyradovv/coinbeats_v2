...
import { useEducatorQuery } from '@/store/api/educator.api'
import { Avatar } from '@radix-ui/react-avatar'
import { List, Recycle, Send, X } from 'lucide-react'
import Image from 'next/image'
import Link from 'next/link'
import { useParams } from 'next/navigation'
import EducatorSkeleton from './educatorSkleton'
import { TEducatorSingle } from '@/types/educator'
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs'
import { useState } from 'react'
import { Button } from '@/components/ui/button'

interface TTabsProps {
    educator: TEducatorSingle | undefined
}

function ActTypes({ educator }: TTabsProps) {
                            ...
                            <div className="grid grid-cols-1 lg:flex lg:justify-between lg:px-6 gap-4 lg:gap-0">
                                ...
                                <div className="lg:w-1/3 lg:flex lg:flex-col lg:items-start">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {educator?.categories?.length ? (
                                            educator.categories.map((e) => (
                                ...
                                <div className="col-span-2 lg:col-span-1 lg:w-1/3 lg:flex lg:flex-col lg:items-end">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {educator?.chains?.length ? (
                                            educator.chains.map((e) => (
...
export default function Educator() {
    ...
    const id = params.educatorId

    const { currentData: educator, isLoading, isFetching } = useEducatorQuery(id as string, { skip: !id })
                ...
                <div className="container mx-auto pt-4 pb-8 px-4">
                    <div className="mb-4 relative overflow-hidden rounded-lg">
                        ...
                        <div className="inset-0 z-0">
                            ...
                            >
                                <Image
                                    src={constructImageUrl(educator?.coverPhotoUrl)}
                                    alt="Background"
                        ...
                        <div className="p-8 relative z-20 flex flex-col items-center justify-center">
                            <Avatar className="h-18 w-18">
                                <AvatarImage src={constructImageUrl(educator?.logoUrl)} alt="Logo" />
                                <AvatarFallback className="text-xl">{educator?.name?.slice(0, 2)}</AvatarFallback>
                            </Avatar>

                            <h2 className="text-2xl font-bold mb-3 text-white">{educator?.name}</h2>
