    ...
    return (
        ...
        >
            ...
            <div className="relative h-36 w-full">
                ...
                <div className="absolute -bottom-10 left-1/2 -translate-x-1/2">
                    <Avatar className="h-20 w-20 border-[4px] border-white shadow-lg">
                        <AvatarImage src={constructImageUrl(podcast.logoUrl)} alt={podcast.name} />
                        <AvatarFallback className="text-xl font-semibold">{podcast.name.slice(0, 2).toUpperCase()}</AvatarFallback>
                    </Avatar>
                </div>
            </div>
            ...
            <CardContent className="mt-14 px-6 text-center flex flex-col gap-3">
                <CardTitle className="text-xl font-bold text-foreground">{podcast.name}</CardTitle>
                <CardDescription className="text-muted-foreground text-sm truncate">
                    {podcast.description || 'This podcast has not provided a bio yet.'}
                </CardDescription>
                ...
                <div className="grid grid-cols-2 gap-4 text-xs text-muted-foreground pt-2">
                    <div className="flex flex-col items-center">
                        ...
                        <span>{podcast._count.chains ?? 0} </span>
                        <span>Chains</span>
                    </div>
                    <div className="flex flex-col items-center">
                        <Layers className="h-4 w-4 mb-1" />
                        <span>{podcast._count.categories ?? 0}</span>
