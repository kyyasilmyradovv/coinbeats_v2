...
import { Page, Button, ListInput, List, BlockTitle, Card, Notification, Popover, Preloader, Block, ListItem, Toggle } from 'konsta/react'
...
const AddTasksForContentPage: React.FC = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        repeatInterval: '',
        displayLocation: 'CONTENT_PAGE', // matches design from AddQuestsPage
        platform: 'NONE',
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const dataToSend = {
                ...
                // Pass correct foreign key based on contentType
            ...
            if (editingTask) {
                ...
                const response = await axios.post('/api/verification-tasks/content', dataToSend)
    ...
    return (
        <Page>
            ...
            {/* Replicate design like AddQuestsPage */}
            <div className="text-center flex w-full items-center justify-center absolute top-8 !mb-18">
                <BlockTitle large>{editingTask ? `Edit ${contentType} Task` : `Add Task to ${contentType}`}</BlockTitle>
            </div>
                ...
                <Block>
                    <Card className="!mb-2 !p-0 !rounded-2xl shadow-lg !mx-2 relative border border-gray-300 dark:border-gray-600">
                        <List strong inset className="!m-0 !p-0">
                            ...
                            {/* Short Circuit as a styled toggle */}
                            <ListItem
                                title="Short Circuit"
                                after={
                                    <Toggle
