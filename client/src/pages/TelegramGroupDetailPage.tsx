...
const TelegramGroupDetailPage: React.FC = () => {
    ...
    }, [activeTab, group, fetchUserVerificationTasks])

    const getGroupTasks = async (groupId: number) => {
        setLoadingTasks(true)
        try {
            const url = `/api/verification-tasks/content/TelegramGroup/${groupId}`
            const response = await axios.get(url)
            setTasks(response.data)
        } catch (error) {
            console.error('Error fetching TelegramGroup tasks:', error)
        } finally {
    ...
    const renderMainCard = () => {
        ...
        return (
            <div className="!p-0 mx-4 mt-4 rounded-xl shadow-lg overflow-hidden bg-white dark:bg-zinc-900 border border-gray-300 dark:border-gray-600 !mb-20">
                ...
                {activeTab === 'overview' && (group.logoUrl || group.name) && (
