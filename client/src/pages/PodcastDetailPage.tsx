...
const PodcastDetailPage: React.FC = () => {
    ...
    // ==========================
    useEffect(() => {
        if (activeTab === 'tasks' && podcast) {
            getPodcastTasks(podcast.id)
            fetchUserVerificationTasks()
        }
    }, [activeTab, podcast, fetchUserVerificationTasks])

    const getPodcastTasks = async (podcastId: number) => {
        setLoadingTasks(true)
        try {
            const url = `/api/verification-tasks/content/Podcast/${podcastId}`
            const response = await axios.get(url)
            setTasks(response.data)
        } catch (error) {
            console.error('Error fetching Podcast tasks:', error)
        } finally {
